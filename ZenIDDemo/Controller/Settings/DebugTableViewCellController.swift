...
final class DebugTableViewCellController: TableCellController {
    ...
    func view(in tableView: UITableView, at indexPath: IndexPath) -> UITableViewCell {
        ...
        cell.title.text = NSLocalizedString("settings-verifier-debug", comment: "")
        cell.valueSwitch.isOn = service.load().isDebugEnabled
    ...
    private func valueSwitchDidChange(sender: UISwitch) {
        try? service.update(isDebugEnabled: sender.isOn)
    }
