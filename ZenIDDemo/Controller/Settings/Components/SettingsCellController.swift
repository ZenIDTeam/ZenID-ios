...
final class SettingsCellController: TableCellController {
    private let viewModel: SettingsValueViewModel
    private weak var valueLabel: UILabel?

    init(viewModel: SettingsValueViewModel) {
        self.viewModel = viewModel
    }

    func view(in tableView: UITableView, at indexPath: IndexPath) -> UITableViewCell {
        let cell = tableView.dequeueReusableCell(withIdentifier: String(describing: IntervalTableViewCell.self), for: indexPath) as! IntervalTableViewCell
