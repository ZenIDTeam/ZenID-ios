...
function Visualizer(options = {}) {
    ...
    // Time in ms when the focus animation was requested first.
    // A single frame where the focus animation is not requested will reset this.
    this.focusAnimationStartDate = null;

    // Used for fading in the outline
    this.showOutlineStartDate = null;

    this.drawOverlay = function (canvas, age, renderCommands) {
    ...
    this.init = function () {
        focusAnimationStartDate = null;
        showOutlineStartDate = null;
    }

    function getCssProperty(specificCssProperty, generalCssProperty, fallback) {
        return self.rootStyle.getPropertyValue(specificCssProperty) || self.rootStyle.getPropertyValue(generalCssProperty) || fallback;
    }
    ...
    const widgets = {
        ...
        card_outline: function (context, age, widget) {
            ...
            // Render the focus animation. It's rounded corners of a rectangle inside the card outline.
            // Corners animated to become bigger and smaller.

            if (!widget.enableFocusAnimation) {
                self.focusAnimationStartDate = null;
            }

            if (widget.enableFocusAnimation && self.focusAnimationStartDate === null) {
                self.focusAnimationStartDate = Date.now();
            }

            const focusAnimationDelayMs = 5000;
            if (self.focusAnimationStartDate !== null && Date.now() - self.focusAnimationStartDate > focusAnimationDelayMs) {
                const delayedStartDate = self.focusAnimationStartDate + focusAnimationDelayMs;
                const t = (Date.now() - delayedStartDate) / 1000;
                const margin = 0.15 + 0.075 * Math.sin(t);
                const focusRect = {
                    x: rect.x + rect.width * margin,
                    y: rect.y + rect.height * margin,
                    width: rect.width * (1 - margin * 2),
                    height: rect.height * (1 - margin * 2),
                }
                roundedCornerPath(context, focusRect, cornerRadius, false);
                context.stroke();
            }
        },
        ...
        face_liveness_checkboxes: function (context, age, widget) {

            if (!images.statusError) {
                images.statusError = new Image();
                images["statusError"].src = getCssProperty("--status-error-image-face-liveness", "--status-error-image",
                    `${visualizerScriptDir}/Images/Status=Error, Background=Fill.svg`);
            }

            if (!images.statusSuccess) {
                images["statusSuccess"] = new Image();
                images["statusSuccess"].src = getCssProperty("--status-success-image-face-liveness", "--status-success-image",
                    `${visualizerScriptDir}/Images/Status=Success, Background=Fill.svg`);
            }

            if (!images.statusWaiting) {
                images["statusWaiting"] = new Image();
                images["statusWaiting"].src = getCssProperty("--status-waiting-image-face-liveness", "--status-waiting-image",
