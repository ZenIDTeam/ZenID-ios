...
public class SecureMessaging {
    ...
    func protect(apdu : NFCISO7816APDU ) throws -> NFCISO7816APDU {
        ...
        ApplicationLogger.shared.Verbose("\t\tSSC: " + binToHexRep(self.ssc))
        self.ssc = self.incSSC()
        let paddedSSC = algoName == .DES ? self.ssc : [UInt8](repeating: 0, count: 8) + ssc
        ApplicationLogger.shared.Verbose("\tIncrement SSC with 1")
        ApplicationLogger.shared.Verbose("\t\tSSC: " + binToHexRep(self.ssc))
        ...
        ApplicationLogger.shared.Verbose(tmp)
        ApplicationLogger.shared.Verbose("\tM: " + binToHexRep(M))
        
        ApplicationLogger.shared.Verbose("Compute MAC of M")
        
        let N = pad(paddedSSC + M, blockSize:padLength)
        ApplicationLogger.shared.Verbose("\tConcatenate SSC and M and add padding")
        ApplicationLogger.shared.Verbose("\t\tN: " + binToHexRep(N))

        var CC = mac(algoName: algoName, key: self.ksmac, msg: N)
        if CC.count > 8 {
            CC = [UInt8](CC[0..<8])
        }
        ApplicationLogger.shared.Verbose("\tCompute MAC over N with KSmac")
        ApplicationLogger.shared.Verbose("\t\tCC: " + binToHexRep(CC))
        ...
        ApplicationLogger.shared.Verbose("Construct and send protected APDU")
        ApplicationLogger.shared.Verbose("\tProtectedAPDU: " + binToHexRep(protectedAPDU))
    ...
    func unprotect(rapdu : ResponseAPDU ) throws -> ResponseAPDU {
        ...
        ApplicationLogger.shared.Verbose("Receive response APDU of MRTD's chip")
        ApplicationLogger.shared.Verbose("\tRAPDU: " + binToHexRep(rapduBin))
        ...
        guard rapduBin.count >= offset + 5 else {
            ApplicationLogger.shared.Error("size error")
        ...
        if rapduBin[offset] == 0x8E {
            ...
            ApplicationLogger.shared.Verbose("Verify RAPDU CC by computing MAC of" + tmp)
            
            let K = pad(paddedSSC + do87 + do99, blockSize:padLength)
            ApplicationLogger.shared.Verbose("\tConcatenate SSC and" + tmp + " and add padding")
            ApplicationLogger.shared.Verbose("\t\tK: " + binToHexRep(K))
            
            ApplicationLogger.shared.Verbose("\tCompute MAC with KSmac")
            var CCb = mac(algoName: algoName, key: self.ksmac, msg: K)
            if CCb.count > 8 {
                CCb = [UInt8](CC[0..<8])
            }
            ApplicationLogger.shared.Verbose("\t\tCC: " + binToHexRep(CCb))
            
            let res = (CC == CCb)
            ApplicationLogger.shared.Verbose("\tCompare CC with data of DO'8E of RAPDU")
            ApplicationLogger.shared.Verbose("\t\t\(binToHexRep(CC))  == \(binToHexRep(CCb)) ? \(res)")
        ...
        if do87Data.count > 0 {
            ...
            // There is a payload
            data = unpad(dec)
            ApplicationLogger.shared.Verbose("Decrypt data of DO'87 with KSenc")
            ApplicationLogger.shared.Verbose("\tDecryptedData: " + binToHexRep(data))
        }
        
        ApplicationLogger.shared.Verbose("Unprotected APDU: [\(binToHexRep(data))] \(binToHexRep(sw1)) \(binToHexRep(sw2))" )
        return ResponseAPDU(data: data, sw1: sw1, sw2: sw2)
    }

    func maskClassAndPad(apdu : NFCISO7816APDU ) -> [UInt8] {
        ApplicationLogger.shared.Verbose("Mask class byte and pad command header")
        let res = pad([0x0c, apdu.instructionCode, apdu.p1Parameter, apdu.p2Parameter], blockSize: padLength)
        ApplicationLogger.shared.Verbose("\tCmdHeader: " + binToHexRep(res))
        return res
    }
    ...
    func buildD087(apdu : NFCISO7816APDU) throws -> [UInt8] {
        ...
        ApplicationLogger.shared.Verbose("Build DO'87")
        ApplicationLogger.shared.Verbose("\tDO87: " + binToHexRep(res))
        return res
    }
    ...
    func padAndEncryptData(_ apdu : NFCISO7816APDU) -> [UInt8] {
        ...
        ApplicationLogger.shared.Verbose("Pad data")
        ApplicationLogger.shared.Verbose("\tData: " + binToHexRep(paddedData))
        ApplicationLogger.shared.Verbose("Encrypt data with KSenc")
        ApplicationLogger.shared.Verbose("\tEncryptedData: " + binToHexRep(enc))
        return enc
    }
    ...
    func buildD08E(mac : [UInt8]) -> [UInt8] {
        ...
        ApplicationLogger.shared.Verbose("Build DO'8E")
        ApplicationLogger.shared.Verbose("\tDO8E: \(binToHexRep(res))" )
        return res
    }
    ...
    func buildD097(apdu : NFCISO7816APDU) throws -> [UInt8] {
        ...
        ApplicationLogger.shared.Verbose("Build DO'97")
        ApplicationLogger.shared.Verbose("\tDO97: \(res)")
        return res
    }
}
