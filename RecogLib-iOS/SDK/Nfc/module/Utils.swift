...
public func desMAC(key: [UInt8], msg: [UInt8]) -> [UInt8] {
    ...
    ApplicationLogger.shared.Verbose("Calc mac")
    for i in 0 ..< size {
        let tmp = [UInt8](msg[i * 8 ..< i * 8 + 8])
        ApplicationLogger.shared.Verbose("x\(i): \(binToHexRep(tmp))")
        y = DESEncrypt(key: [UInt8](key[0 ..< 8]), message: tmp, iv: y)
        ApplicationLogger.shared.Verbose("y\(i): \(binToHexRep(y))")
    }

    ApplicationLogger.shared.Verbose("y: \(binToHexRep(y))")
    ApplicationLogger.shared.Verbose("bkey: \(binToHexRep([UInt8](key[8 ..< 16])))")
    ApplicationLogger.shared.Verbose("akey: \(binToHexRep([UInt8](key[0 ..< 8])))")
    let iv: [UInt8] = [0, 0, 0, 0, 0, 0, 0, 0]
    let b = DESDecrypt(key: [UInt8](key[8 ..< 16]), message: y, iv: iv, options: UInt32(kCCOptionECBMode))
    ApplicationLogger.shared.Verbose("b: \(binToHexRep(b))")
    let a = DESEncrypt(key: [UInt8](key[0 ..< 8]), message: b, iv: iv, options: UInt32(kCCOptionECBMode))
    ApplicationLogger.shared.Verbose("a: \(binToHexRep(a))")
